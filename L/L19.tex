...
\begin{remark}
{\color{blue}\[y(t) = y_H(t) + y_p(t), t \in I.\]}
\end{remark}
...
{\color{blue}\[a_{n+2} = \frac{2 a_n}{n+2}, \ n = 0, 1, 2, \cdots\]}
...
Then,
{\color{blue}\[y_1(t) = \sum_{k=0}^{\infty} a_{2k} t^{2k} = \sum_{k=0}^{\infty} \frac{1}{k!} (t^2)^k = e^{t^2}\]}
...
Then,
{\color{blue}\[y_2(t) = \sum_{n=0}^{\infty} a_n t^n = \sum_{k=0}^{\infty} a_{2k+1} t^{2k+1}\]}
...
{\color{blue}Then \(\{y_1, y_2\}\) is a basis.}
\end{example}
...
{\color{blue}We say that the eq \(p(t) \frac{d^2 y}{dt^2} + q(t) \frac{dy}{dt} + r(t) y = 0\) is singular at \(t=t_0\) if \(p(t_0) = 0\).}

\begin{example}[Euler's equation]
Let's find a basis of solutions to the Euler's eq.
{\color{blue}\[t^2 \frac{d^2 y}{dt^2} + \alpha t \frac{dy}{dt} + \beta y = 0\]}
...
{\color{blue}\[r_{1,2} = \frac{-(\alpha-1) \pm \sqrt{(\alpha-1)^2 - 4\beta}}{2}\]}

\begin{itemize}
\item {\color{blue}Assume that \((\alpha-1)^2 - 4\beta > 0\). Then, \(y_1(t) = t^{r_1}\) and \(y_2(t) = t^{r_2}\) are solutions for \(t > 0\).} Also,
\[W(y_1, y_2)(t) = \det \begin{pmatrix} t^{r_1} & t^{r_2} \\ r_1 t^{r_1-1} & r_2 t^{r_2-1} \end{pmatrix} = r_2 t^{r_1+r_2-1} - r_1 t^{r_1+r_2-1} \neq 0, \ \forall t > 0\]
since \(r_1 \neq r_2\).
    
\item {\color{blue}Assume that \((\alpha-1)^2 - 4\beta = 0\).}
Then \(r_1 = r_2 = r = \frac{1-\alpha}{2}\). Thus, {\color{blue}\(y_1(t) = t^r\)} is one solution for \(t > 0\). {\color{blue}We'll look for a second solution by a method often called \textbf{reduction of order}. We look for a solution \(y_2(t) = u(t) t^r\).} Observe that
\begin{align*}
...
{\color{blue}Then, \(u(t) = \ln t\), \(t > 0\). Then \(y_2(t) = (\ln t) t^r\).}
