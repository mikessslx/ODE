...
\end{align*}
i.e.
{\color{blue}\[m \frac{d^2 y}{dt^2} + c \frac{dy}{dt} + ky = F \tag{20}\]}
\end{example}
...
{\color{blue}\[y(t) = c_1 \cos(\omega_0 t) + c_2 \sin(\omega_0 t) \ t \geq 0, \ c_1, c_2 \in \mathbb{R}\]}

Assume that \(c_1 \neq 0\). {\color{blue}Then, \(y(t) = R \cos(\omega_0 t - \delta)\), where \(\delta \in (-\frac{\pi}{2}, \frac{\pi}{2})\), \(\tan \delta = \frac{c_2}{c_1}\) and \(R = \sqrt{c_1^2 + c_2^2}\) (exercise).} Observe that \(y\) is bounded, more precisely, \(|y(t)| \leq R\) for all \(t \geq 0\). Also, \(y\) is periodic with period \(\frac{2\pi}{\omega_0}\). The solution behaves in the same way if \(c_1 = 0\). {\color{blue}This type of motion is called simple harmonic motion.} Also,
...
\begin{itemize}
\item {\color{blue}Assume \(c^2 - 4mk > 0\).} Then, the roots are
\[0 > \lambda_1 = \frac{-c + \sqrt{c^2 - 4mk}}{2m} \ \text{and} \ \lambda_2 = \frac{-c - \sqrt{c^2 - 4mk}}{2m} < 0\]

Hence, the general solution is
{\color{blue}\[\underset{\to 0 \text{ as } t \to +\infty}{y(t)} = c_1 e^{\lambda_1 t} + c_2 e^{\lambda_2 t}, \ t \geq 0, \ c_1, c_2 \in \mathbb{R}\]}

{\color{blue}This motion is known as \textbf{overdamped motion}.}

\item {\color{blue}Assume that \(c^2 - 4mk = 0\).}
The char. polynomial has a double root \(\lambda = -\frac{c}{2m}\).
The general solution is
{\color{blue}\[\underset{\to 0 \text{ as } t \to +\infty}{y(t)} = c_1 e^{-\frac{c t}{2m}} + c_2 t e^{-\frac{c t}{2m}}, \ t \geq 0, \ c_1, c_2 \in \mathbb{R}\]}
    
{\color{blue}This motion is known as \textbf{critically damped motion}.}
...
The general solution is
{\color{blue}\[\underset{\to 0 \text{ as } t \to +\infty}{y(t)} = c_1 e^{-\frac{c t}{2m}} \cos(\mu t) + c_2 e^{-\frac{c t}{2m}} \sin(\mu t), \ t \geq 0, \ c_1, c_2 \in \mathbb{R},\]
where \(\mu = \frac{1}{2m} \sqrt{4mk - c^2}\).}

{\color{blue}This motion is called \textbf{damped vibrations}.} \sout{Observe that, for \(c_1 \neq 0\),}
{\color{blue}\[y(t) = e^{-\frac{ct}{2m}} R \cos(\mu t - \delta) \ \text{and} \ y(t) = e^{-\frac{ct}{2m}} c_2 \cos(\mu t - \pi/2) \ \text{if } c_1 = 0.\]}
\end{itemize}
...
{\color{blue}\[y_p(t) = \frac{F_0 \cos(\omega t - \delta)}{((k - m\omega^2)^2 + c^2 \omega^2)^{1/2}} \ \text{where } \tan \delta = \frac{c\omega}{k - m\omega^2}\]
is a particular solution provided \(\underbrace{k \neq m\omega^2}_{\omega \neq \sqrt{\frac{k}{m}} = \omega_0}\), in other words, \(\omega \neq \omega_0\).}

{\color{blue}As \(y_H(t) \to 0\) as \(t \to +\infty\), we observe that \(y(t)\) behaves as \(y_p(t)\) as \(t \to +\infty\).}
...
{\color{blue}\[y_p(t) = \frac{F_0 \cos(\omega t)}{m(\omega_0^2 - \omega^2)}\]
...
{\color{blue}\[y_p(t) = \frac{F_0 t}{2m\omega_0} \sin(\omega_0 t)\]
...
{\color{blue}Here we say that the external force is in resonance with the natural frequency of the system.}
