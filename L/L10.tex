...
\begin{recap}
\begin{itemize}
\item \(\frac{dy}{dt} = A(t)y + b(t)\), \(b = 0\).
        
The set of solutions is a vector space of dimension \(n\).
\begin{itemize}
\item \(\mathcal{B} = \{y^{(1)}, y^{(2)}, \cdots, y^{(n)}\}\);
\item \(y_H = c_1 y^{(1)} + c_2 y^{(2)} + \cdots + c_n y^{(n)}\), \(c_1, \cdots, c_n \in \mathbb{R} \rightsquigarrow\) general solution to \(\frac{dy}{dt} = A(t)y\).
\end{itemize}
        
\(\mathcal{B}\) is a basis of solutions.

\item \(\frac{dy}{dt} = A(t)y + b(t)\), \(b \not\equiv 0\).
\begin{itemize}
\item \(\mathcal{B}, y_H\) as before;
\item \(y = y_H + y_p\), where \(y_H\) is the general solution to the associated homogeneous system \(\frac{dy}{dt} = A(t)y\), and \(y_p\) is a particular solution to \(\frac{dy}{dt} = A(t)y + b(t)\).
\end{itemize}
\end{itemize}
\end{recap}
...
\leavevmode
\begin{itemize}
\item \(\frac{dy}{dt} = y\), \(n=1\),
\[y(t) = C e^t, \ y_1(t) = 2e^t, \ y_2(t) = -\frac{9}{11} e^t.\]
\item \(\frac{dy}{dt} = y, \ y(0) = 1\).
\end{itemize}
\end{example}

\begin{remark}
\(\frac{dy}{dt} = \frac{dy_H}{dt} + \frac{dy_p}{dt} = A(t)y_H + A(t)y_p + b(t) = A(t)(y_H + y_p) + b(t) = A(t)y + b(t)\).
\end{remark}
...
The function \(y(t) = C_1(t)y^{(1)}(t) + \cdots + C_n(t)y^{(n)}(t)\) is a solution to \(\frac{dy}{dt} = A(t)y + b(t)\) if \(C_1, \cdots, C_n\) are continuously differentiable functions in \(I\), such that \(Q(t) \frac{dC}{dt} = b(t)\) where \(Q\) is the fundamental matrix.
