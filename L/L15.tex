...
\[y(t) = e^{At} \left( e^{-At_0} y(t_0) + \int_{t_0}^t e^{-As} b(s) ds \right)\]
...
That is,
\[y(t) = e^{A(t-t_0)} y(t_0) + \int_{t_0}^t e^{A(t-s)} b(s) ds, \ \forall t \in \mathbb{R}\]

(This can be generalized to
\[\frac{dy}{dt} = Ay + b(t), \ A: V \to W\]

\(V, W\) are function spaces of (possible) infinite dimension \(\rightsquigarrow\) Cf Semigroup theory for PDE.)
...
\subsection{§5. Solution Methods for Linear Second-Order Equations}
...
\begin{example}
    ...
    \begin{enumerate}[a)]
        \item
        The equation \(m \frac{d^2 y}{dt^2} = F(t, y, \frac{dy}{dt})\) is the Newton's second law of motion.
        
        The force \(F\) can be nonlinear.
        
        \item
        The equation \(\frac{d^2 \theta}{dt^2} + \frac{g}{L} \sin(\theta) = 0\) is the pendulum equation.

        \item
        The equation \(t^2 \frac{d^2 y}{dt^2} + \alpha t \frac{dy}{dt} + \beta y = 0, \ \alpha, \beta \in \mathbb{R}\), is the Euler's equation.

        \item
        The eq. \(x^2 \frac{d^2 y}{dx^2} + x \frac{dy}{dx} + (x^2 - \nu^2)y = 0\) is the Bessel's eq.
    \end{enumerate}
\end{example}
...
Let \(v_1\) and \(v_2\) be eigenvector for \(\lambda_1\) and \(\lambda_2\), respectively. The functions
...
If \(v = u + iw, \ u, w \in \mathbb{R}^n\), is an eigenvector for \(\lambda\), then the functions:
