...
\item Classification of ODEs:
\begin{itemize}
\item Order
\item ODE (scalar), Systems of ODEs
\item Linear and nonlinear eqs.
\end{itemize}
...
\item Stability (or continuous dependence on data)
\item Ex: \(\begin{cases} y' = f(t, y) \\ y(t_0) = y_0 \end{cases}\)
\end{itemize}
\end{itemize}
\end{recap}
...
For simplicity, we set \(C = 1\), so the integrating factor \(\mu(t) = e^{\int a(t) dt}\).
...
In summary, we deduced that if eq. (6) has a solution, then the solution must be a function in the form (10).
...
The function \(\mu(t) = e^{\int a(t) dt}\) is called an integrating factor for eq. (6).
\end{definition}
