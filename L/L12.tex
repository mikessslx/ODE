...
\begin{recap}
\begin{itemize}
\item \sout{\(\frac{dy}{dt} = Ay + b, \ A \in \mathbb{R}^{n \times n}\)}
\item \sout{\(\frac{dy}{dt} = Ay\)}
\item \sout{If \(n=1\) then the system is one single eq., \(\frac{dy}{dt} = ay\). The general solution is \(y(t) = C e^{at}, \ t \in \mathbb{R}, \ C \in \mathbb{R}\).}
\item \sout{Based on this, we look for solutions to \(\frac{dy}{dt} = Ay\) in the form \(y(t) = v e^{\lambda t}\) where \(v \in \mathbb{R}^n, \ \lambda \in \mathbb{R}\). Also, \(y \not\equiv 0\). Solutions in this form exist provided that \(\lambda\) is a real eigenvalue of \(A\) with eigenvector \(v\).}
\end{itemize}
\end{recap}

\subsection{Case 1: \(A\) has \(n\) distinct real eigenvalues}

{\color{blue}If \(v_k\) is an eigenvector to \(A\) associated to \(\lambda_k\), \(v_k \in \mathbb{R}^n\), then \(y^{(k)}(t) = v_k e^{\lambda_k t}\), \(k=1, \cdots, n\) form a basis of solutions.}

\subsection{Case 2: \(A\) has complex eigenvalues}

\sout{Assume that \(\lambda = \alpha + i\beta \in \mathbb{C}, \ \beta \neq 0\) is a complex eigenvalue of \(A\). Let \(v = u + iw \in \mathbb{C}^n\) be an eigenvector of \(A\) for \(\lambda\), where \(u \in \mathbb{R}^n\) and \(w \in \mathbb{R}^n\).}

\sout{Recall that \(e^z = e^a (\cos(b) + i \sin(b))\) if \(z = a + ib\) and if \(f(t) = e^{zt}\) where \(t \in \mathbb{R}, z \in \mathbb{C}\), then}
\[\frac{df(t)}{dt} = z e^{zt}\]

\sout{Then \(y(t) = v e^{\lambda t}\) is a solution, which takes complex values.}
...
Then \(Ax_1 = \frac{dx_1}{dt}\) and \(Ax_2 = \frac{dx_2}{dt}\). {\color{blue}Thus, \(\underbrace{\text{Re}(y(t))}_{y^{(1)}(t)}\) and \(\underbrace{\text{Im}(y(t))}_{y^{(2)}(t)}\) are real-valued solutions. Also,}
\begin{align*}
...
{\color{blue}Finally, observe that \(u = \frac{v + \bar{v}}{2}\) and \(w = \frac{v - \bar{v}}{2i}\).}
...
{\color{blue}A basis of solutions is given by}
\begin{align*}
...
\end{align*}
{\color{blue}where \(v = u + iw\) is an eigenvector for \(\lambda\).}
