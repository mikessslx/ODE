...
{\color{blue}Specifically, let's look for a solution in the form \(u(x, t) = X(x) T(t)\).}
Replacing \(u\) in the PDE, we get (formally)
\begin{align*}
X T' - k X'' T &= 0 \\
{\color{blue}\underbrace{\frac{1}{k} \frac{T'}{T}}_{\text{function of } t} = \underbrace{\frac{X''}{X}}_{\text{function of } x} = -\lambda} \ \text{for some } \lambda \in \mathbb{R}
\end{align*}
...
X'' + \lambda X = 0 & 0 < x < L \\
X(0) = X(L) = 0
\end{cases} \ \text{and} \ {\color{blue}T' + \lambda k T = 0} \ t > 0.\]}
...
Then \(\sin(\sqrt{\lambda}L) = 0\), i.e. \(\sqrt{\lambda}L = n\pi\), i.e. {\color{blue}\(\lambda_n = (\frac{n\pi}{L})^2\), \(n=1, 2, \cdots\).}

So {\color{blue}\(X_n(x) = \sin(\frac{n\pi}{L}x)\)} is a nontrivial solution, \(n=1, 2, \cdots\).
\end{itemize}
...
Then, {\color{blue}\(u_n(x, t) = e^{-k (\frac{n\pi}{L})^2 t} \sin(\frac{n\pi}{L}x) \ n=1, 2, \cdots\)}
...
{\color{blue}\[u(x, t) = \sum_{n=1}^\infty B_n u_n(x, t)\]}
...
{\color{blue}\[\int_0^L f(x) \sin(\frac{\pi n}{L}x) dx = \sum_{m=1}^\infty B_m \int_0^L \sin(\frac{\pi n}{L}x) \sin(\frac{\pi m}{L}x) dx \overset{\text{delicate step}}{=} \cdots\]}
\end{proof}
...
{\color{blue}\(L[y] = -\frac{d^2 y}{dt^2}\)}, then we call {\color{blue}\(L\)} an operator.
\end{remark}
...
is called an {\color{blue}\textbf{eigenfunction}} of \(L\) with eigenvalue {\color{blue}\(\lambda\)}.
Also, the function \(r\) is called a {\color{blue}\textbf{weight}}.
...
\begin{enumerate}[i)]
\item {\color{blue}All eigenvalues of \(A\) are real.}
\item {\color{blue}There exists a basis of \(\mathbb{R}^n\) of eigenvectors of \(A\).}
\item {\color{blue}If \(v\) and \(w\) are eigenvector with different eigenvalues then \(v \cdot w = 0\).}
\item {\color{blue}\((Av, w) = (v, A^* w) \ \forall v, w \in \mathbb{C}^n\).}
\end{enumerate}
\end{itemize}
\end{recap}
