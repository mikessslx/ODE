...
\begin{recap}
    Under the assumptions given in the theorems we proved:
    \begin{itemize}
        \item \(\frac{dy}{dt} = A(t)y \rightsquigarrow\) The set of solutions is a vector space of dimension \(n\). General solution:
        \[y_H(t) = C_1 y^{(1)}(t) + \cdots + C_n y^{(n)}(t),\]
        where \(\{y^{(1)}, \cdots, y^{(n)}\}\) is a basis of solutions.

        \item \(\frac{dy}{dt} = A(t)y + b(t) \rightsquigarrow\) General solution:
        \[y(t) = y_H(t) + z(t),\]
        where \(z\) is a particular solution to the given system.
        \item We can find \(z\) by the method of variation of parameters
        \[\rightsquigarrow z(t) = C_1(t) y^{(1)}(t) + \cdots + C_n(t) y^{(n)}(t),\]
        where \(C = \begin{pmatrix} C_1 \\ \vdots \\ C_n \end{pmatrix}\) satisfies \(\frac{dC(t)}{dt} = (Q(t))^{-1} b(t)\).
    \end{itemize}
\end{recap}

\subsection{Equations of order \(n\)}

The general form of an ODE of order \(n\) is
\[\frac{d^n y}{dt^n} = f\left( t, y, \frac{dy}{dt}, \frac{d^2 y}{dt^2}, \cdots, \frac{d^{n-1} y}{dt^{n-1}} \right) \tag{20}\]
where \(f\) is given.

Observe the following. Assume that \(y\) is a solution to (20).

Let \(y_1 = y, \ y_2 = \frac{dy}{dt}, \ y_3 = \frac{d^2 y}{dt^2}, \ \cdots, \ y_n = \frac{d^{n-1} y}{dt^{n-1}}\).
Then,
\[\begin{cases}
...
\begin{remark}
    ...
    \begin{enumerate}[a)]
        ...
        \item The general form of a linear eq. of order \(n\) is
...
\[W(y_1, \cdots, y_n)(t) = \det \begin{pmatrix}
y_1(t) & \cdots & y_n(t) \\
...
\subsection{§4. Linear Systems of First-Order with Constant Coefficients}

A linear system of first-order with constant coefficients is
...
Noticed that the general solution is given by
...
Then, \(y(t) = v e^{\lambda t}\) is a nontrivial solution of (1) if and only if \(\lambda\) is an eigenvalue of \(A\) with eigenvector \(v\).

\begin{recap}[A brief review on eigenvalues and eigenvectors]
    Let \(A \in \mathbb{R}^{n \times n}\).
    \begin{enumerate}[a)]
        \item We say that \(\lambda \in \mathbb{C}\) is an eigenvalue of \(A\) if there exists \(v \in \mathbb{C}^n \setminus \{0\}\) s.t. \(Av = \lambda v\). In this case, we say that \(v\) is an eigenvector of \(A\) with eigenvalue \(\lambda\).

        \item \(\lambda \in \mathbb{C}\) is an eigenvalue of \(A\) if and only if \(\lambda\) is a root of the characteristic polynomial of \(A\), \(p(\lambda) = \det(A - \lambda I)\).
        \[Av = \lambda v \iff Av - \lambda v = 0 \iff Av - \lambda I v = 0 \iff (A - \lambda I)v = 0\]
        
        \item The matrix \(A\) has at most \(n\) distinct eigenvalues and at most \(n\) l.i. eigenvectors.

        \item If \(v_1, \cdots, v_k \in \mathbb{C}^n\) are eigenvectors of \(A\) associated to different eigenvalues \((\lambda_1, \cdots, \lambda_k \in \mathbb{C})\) then \(v_1, \cdots, v_k\) are linearly independent, for some \(1 \le k \le n\).
    \end{enumerate}
\end{recap}
