...
Recall that the eigenvalues are \(\lambda_n = (n\pi)^2\) and the corresponding eigenfunctions are
\[y_n(t) = \sin(n\pi t), \ n=1, 2, \cdots\]

Consider \(f(t) = t(\pi-t)\), \(t \in [0, \pi]\).

In the previous lecture we proved that the eigenfunction expansion of \(f\) is
\[\sum_{k=0}^\infty \frac{8 \sin((2k+1)t)}{\pi (2k+1)^3}.\]
...
Assume that \(g\) is differentiable in \([0, \pi]\) and \(\frac{dg}{dt} = \sum_{k=0}^\infty \frac{d}{dt} \left( \frac{8 \sin((2k+1)t)}{\pi (2k+1)^3} \right) (*)\).
...
Also, observe that
\[\sum_{k=0}^\infty \frac{8 \cos((2k+1)t)}{\pi (2k+1)^2} \underset{\text{exercise}}{\underset{\uparrow}{=}} \pi - 2t = \frac{df}{dt}.\]
...
\[p = 1, \ q = 0, \ r = 1, \ a_1 = 0, \ b_1 = 1, \ a_2 = 1, \ b_2 = 1.\]

First, let's compute the eigenvalues and eigenfunctions.

\begin{enumerate}
\item Assume \(\lambda = 0\). Then the eq. is \(\frac{d^2y}{dt^2} = 0\), so the general solution is \(y(t) = At + B\), \(A, B \in \mathbb{R}\).
Then \(\frac{dy}{dt}(t) = A\). Also, \(\frac{dy}{dt}(0) = A = 0\), and \(y(1) + \frac{dy}{dt}(1) = B + A = 0\), so \(B = -A = 0\).

\item Assume that \(\lambda < 0\). Then the char. polynomial is \(p(\omega) = \omega^2 + \lambda\). The roots are \(\sqrt{-\lambda}\) and \(-\sqrt{-\lambda}\).
The general solution to the eq. is \(y(t) = A e^{-\sqrt{-\lambda}t} + B e^{\sqrt{-\lambda}t}\). Then,
...
\item Assume that \(\lambda > 0\). The roots of the char. polynomial are \(\sqrt{\lambda} i\) and \(-\sqrt{\lambda} i\). The general solution to the eq. is \(y(t) = A \cos(\sqrt{\lambda}t) + B \sin(\sqrt{\lambda}t)\). Then,
...
\[A (\cos(\sqrt{\lambda}) - \sqrt{\lambda} \sin(\sqrt{\lambda})) = 0 \iff A (1 - \sqrt{\lambda} \tan(\sqrt{\lambda})) = 0.\]

If \(\lambda_n\) is the unique solution to the eq. \(1 - \sqrt{\lambda} \tan \sqrt{\lambda} = 0\) in \((0, \pi/2)\) and \(\lambda_n\) is the unique sol. to \(1 - \sqrt{x} \tan \sqrt{x} = 0\) in \((\frac{\pi}{2} + (n-1)\pi, \frac{\pi}{2} + n\pi)\) then \(y(1) + \frac{dy}{dt}(1) = 0\) for any value of \(A\).
\end{enumerate}

Therefore, the eigenvalues are \(\lambda_n\), \(n \in \mathbb{N}_0\), defined above, and the eigenfunctions are
\[y_n(t) = \cos(\sqrt{\lambda_n} t), \ n \in \mathbb{N}_0.\]
