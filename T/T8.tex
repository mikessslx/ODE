...
{\color{blue}\[\frac{d^2y}{dt^2} - 2\frac{dy}{dt} + y = t\]}
\end{xca}
...
{\color{blue}Then, the functions \(y_1(t) = e^t, \ y_2(t) = te^t, \ t \in \mathbb{R}\), define a basis of solutions.
...
{\color{blue}So \(e^t\) and \(te^t\) are linearly independent in \(\mathbb{R}\).}
\end{remark}

{\color{blue}Let's look for a particular solution \(y_p\) in the form \(y_p(t) = at + b\).}

\begin{remark}
\sout{Consider the system \(\frac{dY}{dt} = AY\), \(Y^{(1)} = \binom{y_1^{(1)}}{y_2^{(1)}}\), \(Y^{(2)} = \binom{y_1^{(2)}}{y_2^{(2)}}\).
...
{\color{blue}A particular solution is \(y_p(t) = t + 2\).}

{\color{blue}Then, \(y(t) = c_1 e^t + c_2 te^t + t + 2, \ t \in \mathbb{R}, \ c_1, c_2 \in \mathbb{R}\).}
\end{proof}
...
{\color{blue}\[m \frac{d^2y}{dt^2} + c \frac{dy}{dt} + ky = F(t)\]}

\begin{itemize}
\item {\color{blue}\(m\): mass of the object}
\item {\color{blue}\(c\): constant related to the medium (\(c \ge 0\))}
\item {\color{blue}\(k\): spring-constant (\(k > 0\))}
\item {\color{blue}\(F\): external force}
\end{itemize}

We'll analyze these cases:
\begin{itemize}
\item {\color{blue}Free vibrations (\(c=0, F=0\))}
\item {\color{blue}Damped free vibrations (\(c \neq 0, F=0\))}
\item {\color{blue}Damped forced vibrations (\(c \neq 0, F \neq 0\))}
\item {\color{blue}Forced free vibrations (\(c=0, F \neq 0\))}
    
\sout{\(\rightsquigarrow\) A special case models resonance. (Tacoma Bridge (1940))}
\end{itemize}
