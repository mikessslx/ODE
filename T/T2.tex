...
\begin{proof}[Solution]
    ...
    Finally, the solution is given implicitly by \(2yt + e^t - \cos(y) = C, \ C \in \mathbb{R}\).
\end{proof}
...
\begin{proof}[Solution]
    ...
    The solution is given by \(y^2e^t + 2ye^{2t} = c, \ c \in \mathbb{R}\).
\end{proof}
...
\begin{proof}[Solution]
    ...
    After solving the eq., we find the solution given by \(-\frac{1}{t} + \frac{1}{y^2}(t-1) = c, \ c \in \mathbb{R}\).
\end{proof}

\begin{remark}
    Suppose we also have the initial condition \(y(2) = 1\).

    From this, \(c = \frac{1}{2}\). The solution is \(y^2 = \frac{t-1}{2+t} 2t\).

    This makes sense iff \(\frac{t-1}{2+t} 2t \geq 0\). From here, we find the domain (exercise).
\end{remark}
