...
Consider the eq.
{\color{blue}\[\frac{d^2y}{dt^2} + \frac{3t}{1+t^2} \frac{dy}{dt} + \frac{1}{1+t^2} y = 0\]}

\begin{itemize}
\item[a)] Find the general solution.
\item[b)] {\color{blue}Find the solution that satisfies the initial conditions \(y(0)=2, \frac{dy}{dt}(0)=3\).}
\end{itemize}
...
{\color{blue}\[(1+t^2) \frac{d^2y}{dt^2} + 3t \frac{dy}{dt} + y = 0\]}
...
\sout{Observe that the coefficient of \(a_n\) is \(n^2 - n + 3n + 1 = n^2 + 2n + 1 = (n+1)^2\).
Thus,}
\[(n+2)(n+1) a_{n+2} + (n+1)^2 a_n = 0\]
i.e.
{\color{blue}\[a_{n+2} = - \frac{n+1}{n+2} a_n, \ n \ge 0\]}

\begin{itemize}
\item \sout{Consider \(a_0=1, a_1=0\).
...
{\color{blue}\[a_{2n} = (-1)^n \frac{1 \cdot 3 \cdot 5 \cdots (2n-1)}{2 \cdot 4 \cdot 6 \cdots (2n)} = \frac{(-1)^n 1 \cdot 3 \cdot 5 \cdots (2n-1)}{2^n n!}\]}
    
The solution is
{\color{blue}\[y_1(t) = \sum_{n=0}^{\infty} \frac{(-1)^n (2n-1)!!}{2^n n!} t^{2n}\]}
    
By ratio test, we have
\[\left| \frac{a_{2n+2} t^{2n+2}}{a_{2n} t^{2n}} \right| = \left| \frac{-(2n+1)}{2n+2} t^2 \right| \xrightarrow{n \to \infty} t^2\]
    
{\color{blue}Then, the series is convergent if \(|t| < 1\) and divergent if \(|t| > 1\).}

\item \sout{Consider \(a_0=0, a_1=1\).
Similarly, we find (exercise),}
{\color{blue}\[y_2(t) = \sum_{n=0}^{\infty} \frac{(-1)^n 2^n n!}{3 \cdot 5 \cdots (2n+1)} t^{2n+1}, \ |t| < 1\]}
\end{itemize}

\sout{Also, {\(W(y_1, y_2)(t) = \det \begin{pmatrix} y_1 & y_2 \\ y_1' & y_2' \end{pmatrix}\).}
If \(t=0\),}
\[W(y_1, y_2)(0) = \det \begin{pmatrix} a_0 & 0 \\ 0 & a_1 \end{pmatrix} = 1 \ne 0.\]

{\color{blue}Thus, \(\{y_1, y_2\}\) is a basis of the set of solutions in \((-1, 1)\).}

\subsubsection{a) Method 2}

\sout{Let's propose a solution in the form} {\color{blue}\(y(t) = (1+t^2)^m\)} \sout{where \(m\) must be determined.
...
{\color{blue}So, \(y_1(t) = (1+t^2)^{-1/2}\) is a solution in \(\mathbb{R}\).}

\sout{Now, we look for a second solution as} {\color{blue}\(y_2(t) = y_1(t) v(t)\)} \sout{where \(v\) must be determined.
...
{\color{blue}\[\frac{dv}{dt} = \frac{1}{(1+t^2)^{1/2}}\]}

Then \(v(t) = \sinh^{-1}(t) = \ln(t + \sqrt{1+t^2})\).

{\color{blue}Thus, \(y_2(t) = (1+t^2)^{-1/2} \ln(t + \sqrt{1+t^2}), \ t \in \mathbb{R}\).}

{\color{blue}The sol. are l.i. (exercise, use the Wronskian), so \(\{y_1, y_2\}\) is a basis of sol. in \(\mathbb{R}\).}

\subsubsection{b)}

{\color{blue}\(y(t) = C_1 (1+t^2)^{-1/2} + C_2 (1+t^2)^{-1/2} \ln(t + \sqrt{1+t^2})\), \(C_1 ?, C_2 ?, t \in \mathbb{R}\).}
\begin{align*}
y(0) &= C_1 = 2 \\
y'(0) &= C_2 = 3
\end{align*}
