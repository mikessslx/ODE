...
\begin{xca}
{\color{blue}A small object of mass \(1 \text{kg}\) is attached to a spring with spring constant \(1 \text{N/m}\) (\(1\text{N} = 1 \text{kg m/s}^2\)) and is immersed in a viscous medium with damping constant \(2 \text{N s/m}\).}
...
{\color{blue}\[\frac{d^2y}{dt^2} + 2 \frac{dy}{dt} + y = 0\]}

(Diagram: spring mass system, \(y(t)\) displacement w.r.t equilibrium, \(y\) axis points down.)

The initial conditions are
\begin{itemize}
\item {\color{blue}Mass lowered \(1/4\) m: \(y(0) = \frac{1}{4}\).}
\item {\color{blue}Upward velocity \(1\) m/s: \(y'(0) = -1\) (since \(y\) is positive downwards).}
\end{itemize}

{\color{blue}The characteristic polynomial is \(p(\lambda) = \lambda^2 + 2\lambda + 1 = (\lambda + 1)^2\).
It has a double root \(\lambda = -1\).}
(Basis: \(y_1(t) = e^{-t}, \ y_2(t) = t e^{-t}\).)
{\color{blue}Then, the general solution to the eq. is
...
{\color{blue}The motion is characterized by \(y(t) = \left( \frac{1}{4} - \frac{3}{4} t \right) e^{-t}, \ t \ge 0\).}
...
{\color{blue}The mass passes through its equilibrium position once, and returns toward equilibrium without oscillating.}

\begin{remark}
{\color{blue}The motion is ``Damped Free Vibrations''. The motion can be overdamped, critically damped (the case of the ex.) or damped vibrations.}
\end{remark}
\end{proof}
...
{\color{blue}\[4 \frac{d^2y}{dt^2} + 64 y = F(t) \iff \frac{d^2y}{dt^2} + 16 y = \frac{F(t)}{4}\]}

{\color{blue}The natural freq. is \(\omega_0 = \sqrt{\frac{64}{4}} = \sqrt{16} = 4\).}

\begin{remark}
{\color{blue}\(m \frac{d^2y}{dt^2} + c \frac{dy}{dt} + y = F_0 \cos(\omega t)\). Resonance if \(\omega = \omega_0\), where \(\omega_0 = \sqrt{\frac{k}{m}}\).}
\end{remark}
...
So the equation is
{\color{blue}\[\frac{d^2y}{dt^2} + 16 y = \frac{3A}{16} \cos(\omega t) + \frac{A}{16} \cos(3\omega t)\]}
...
\item {\color{blue}Case \(\omega = 4/3\) (\(3\omega = 4\)): Similar.
\[y(t) = y_H(t) + \text{const} \cdot \cos(\omega t) + \underbrace{\text{const} \cdot t \sin(4t)}_{\text{resonance}}\]}

\item {\color{blue}Case \(\omega \neq 4\) and \(\omega \neq 4/3\):
\[y(t) = y_H(t) + \text{const} \cdot \cos(\omega t) + \text{const} \cdot \cos(3\omega t)\]
We have no resonance.}
\end{itemize}

{\color{blue}Thus, resonance occurs at \(\omega = 4\) and \(\omega = 4/3\).}

\begin{remark}[Method 2 (Idea)]
Many times, for a general \(F\),
{\color{blue}\[F(t) = \sum_{n=0}^\infty B_n \cos(\delta_n t)\]}
{\color{blue}Resonance occurs if any \(\delta_n = \omega_0\).}
\end{remark}
\end{proof}
