...
\begin{enumerate}
\item[(a)] The nonhom. prob. admits a unique sol. for every \(f \in C[\alpha, \beta]\).
\item[(b)] The associated hom. prob. has non trivial solutions.
\end{enumerate}
\end{theorem}
...
\begin{itemize}
\item Classification: order, scalar eqs / system of eqs, linear and nonlinear eqs.
\item Sol. meth.: meth. of integ. factor for linear eqs., direct integ. for separable eqs., sol. meth. for exact eqs. and meth. of integ. factors for non-exact eqs.
\end{itemize}
...
\begin{aligned}
&\text{Picard-Lindelof } (\exists !) \\
&\text{Cauchy-Peano } (\exists) \\
&\text{Picard iterates.}
\end{aligned}
\end{align*}
...
\item Linear systems, \(\frac{dy}{dt} = A(t)y + b(t)\). Structure of the set of solutions. Solution matrix. Meth. of variation of parameters.
\item Wronskian.
\end{itemize}
...
\[\frac{dy}{dt} = Ay + b\]
\begin{itemize}
\item Basis of sol. to \(\frac{dy}{dt} = Ay\) is related to the eigenval. and eigenvectors of \(A\).
\item Exponential matrix.
\end{itemize}

\paragraph{\S5. Sol. Meth. for Second-Order Linear Eqs.}
\begin{itemize}
\item Const. coeff.: Roots of the char. polynomial and basis.
\item Non-const. coeff.: Series sol. meth.
\end{itemize}
