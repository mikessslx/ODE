...
\color{blue}{By the existence and uniqueness thm. (Picard-Lindel\"of), we know that there exists a unique solution \(y\) on \([0, \alpha]\) where \(\alpha = \min\{a, \frac{b}{M}\}\) and \(M = \max_{(t,y)\in R} |f(t,y)|\).}
...
and
\color{blue}{\[\alpha = \min\left\{ a, \frac{1}{9} \right\}\]}

\color{blue}{Then we set \(a = \frac{1}{9}\) and get \(\alpha = \frac{1}{9}\).}
\end{proof}
...
\color{blue}{As \(|f| \leq K\) in \(R\), we have that \(M \leq K\), so \(\alpha \geq \min\left\{ a, \frac{b}{K} \right\}\), \(a, b\) are arbitrary. In particular, we can make \(a\) and \(b\) as large as desired so \(\alpha\) is as large as we want. In other words, \(y\) is well-defined for \(t \in [t_0, T]\) for every \(T > t_0\). So \(y\) must be defined in \([t_0, +\infty)\).}
\end{proof}
...
\color{blue}{\[\underbrace{\frac{dy}{dt}}_{y'(t)} = t\sqrt{1-y^2} \iff \underbrace{\int \frac{y'(t) dt}{\sqrt{1-y(t)^2}}}_{s = y(t)} = \int t dt \iff \arcsin(y(t)) = \frac{t^2}{2} + C\]}

Then, \(y(t) = \sin(\frac{t^2}{2} + C)\), \(C \in \mathbb{R}\). Also, \(y(0) = \sin(C) = 1\). We choose \(C = \frac{\pi}{2}\).
\color{blue}{Then \(y(t) = \sin(\frac{t^2}{2} + \frac{\pi}{2})\).}
...
\color{blue}{This does not violate the thm. since \(\frac{\partial f}{\partial y}\) cannot be continuous at \((t, 1)\).
