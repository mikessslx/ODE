...
{\color{blue}Then, \(p(\lambda) = 0 \iff |1-\lambda| = 6 \iff \lambda_1 = -5\) or \(\lambda_2 = 7\).}
{\color{blue}As the eigenvalues are real and distinct, we know that \(y^{(1)}(t) = v_1 e^{-5t}\) and \(y^{(2)}(t) = v_2 e^{7t}, \ t \in \mathbb{R}\), is a basis of solutions, where \(v_j\) is an eigenvector for \(\lambda_j, \ j=1, 2\).}
...
{\color{blue}For \(w = 1\), we have \(u = -2\). Then, \(v_1 = \begin{pmatrix} -2 \\ 1 \end{pmatrix}\).}

{\color{blue}Similarly, we get \(v_2 = \begin{pmatrix} 2 \\ 1 \end{pmatrix}\). Then, the general solution is
...
{\color{blue}The roots of \(p(\lambda)\) are \(\lambda_1 = 1, \ \lambda_2 = 1+i, \ \lambda_3 = 1-i\).}
Then, if \(v_j\) is an eigenvector of \(\lambda_j, \ j=1, 2, 3\), we have that
{\color{blue}\[y^{(1)}(t) = v_1 e^t, \ z^{(2)}(t) = v_2 e^{(1+i)t}, \ z^{(3)}(t) = v_3 e^{(1-i)t}, \ t \in \mathbb{R}\]}
...
{\color{blue}If \(w = 1\), then \(v_2 = \begin{pmatrix} 0 \\ 1 \\ -i \end{pmatrix} = \begin{pmatrix} 0 \\ 1 \\ 0 \end{pmatrix} + i \begin{pmatrix} 0 \\ 0 \\ -1 \end{pmatrix}\).}
...
{\color{blue}The general solution is \(y(t) = c_1 y^{(1)}(t) + c_2 y^{(2)}(t) + c_3 y^{(3)}(t), \ t \in \mathbb{R}, \ c_1, c_2, c_3 \in \mathbb{R}\).}

\subsubsection{Solution c)}

{\color{blue}As \(A\) is upper triangular, we have that the eigenvalues of \(A\) are \(\lambda_1 = 1\), with multiplicity 2, and \(\lambda_2 = 2\), with multiplicity one.} The dimension of the eigenspace in each case is 1 (exercise). Also, \(v_1 = \begin{pmatrix} 1 \\ 0 \\ 0 \end{pmatrix}\) is an eigenvector for \(\lambda_1\), and \(v_2 = \begin{pmatrix} 0 \\ 0 \\ 1 \end{pmatrix}\) is an eigenvector for \(\lambda_2\). {\color{blue}Now we look for a solution in the form \(y(t) = e^{At} v\) where \(v\) solves \((A - \lambda_1 I)^m v = 0\) for some \(m \le 2\).}
...
{\color{blue}\[y^{(1)}(t) = \begin{pmatrix} 1 \\ 0 \\ 0 \end{pmatrix} e^t, \ y^{(2)}(t) = \begin{pmatrix} 0 \\ 0 \\ 1 \end{pmatrix} e^{2t}, \ y^{(3)}(t) = e^{At} v = (I + (A-I)t) v e^t\]}
...
{\color{blue}(\((A-I)v = \begin{pmatrix} 0 & 1 & 0 \\ 0 & 0 & 0 \\ 0 & 0 & 1 \end{pmatrix} \begin{pmatrix} 0 \\ 1 \\ 0 \end{pmatrix} = \begin{pmatrix} 1 \\ 0 \\ 0 \end{pmatrix}\), so \(y^{(3)}(t) = \left[ \begin{pmatrix} 0 \\ 1 \\ 0 \end{pmatrix} + \begin{pmatrix} 1 \\ 0 \\ 0 \end{pmatrix}t \right] e^t = \begin{pmatrix} t \\ 1 \\ 0 \end{pmatrix} e^t\).)}
